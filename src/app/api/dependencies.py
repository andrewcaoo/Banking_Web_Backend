...
async def get_current_user(
    ...
    if "@" in token_data.username_or_email:
        user: dict | None = await crud_base_account.get(db=db, email=token_data.username_or_email, is_deleted=False)
    else:
        user = await crud_base_account.get(db=db, username=token_data.username_or_email, is_deleted=False)
...
async def rate_limiter(
    request: Request, db: Annotated[AsyncSession, Depends(async_get_db)], user: BaseAccount | None = Depends(get_optional_user)
) -> None:

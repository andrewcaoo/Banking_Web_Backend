...
class Branch(Base):
    ...
    branch_id: Mapped[int] = mapped_column('branch_id',Integer, primary_key=True, autoincrement=True, nullable=False, unique=True)
    branch_name: Mapped[str] = mapped_column(String(50), nullable=False, index=True)

    
    region_id: Mapped[int] = mapped_column(Integer, ForeignKey("Region.region_id",ondelete='CASCADE'), nullable=False)
